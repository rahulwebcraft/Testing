...
$new_class_name = 'cl_custom_heading_'. str_replace( ".", '-', uniqid("", true) );


if ( apply_filters( 'vc_custom_heading_template_use_wrapper', false ) ) {
	$output .= '<div class="' . esc_attr( $css_class ) . ' '.esc_attr( $new_class_name ).'" ' . implode( ' ', $wrapper_attributes ) . '>';
	$output .= '<' . $font_container_data['values']['tag'] . ' ' . $style . ' >';
	$output .= $text;
	$output .= '</' . $font_container_data['values']['tag'] . '>';
	$output .= '</div>';
} else {
	$output .= '<' . $font_container_data['values']['tag'] . ' ' . $style . ' class="' . esc_attr( $css_class ) . ' '.esc_attr( $new_class_name ).'" ' . implode( ' ', $wrapper_attributes ) . '>';
	$output .= $text;
	$output .= '</' . $font_container_data['values']['tag'] . '>';
}
...
if( $responsive_font_size != '' )
	$custom_inline_css .= '@media (max-width:767px){ .' . esc_attr( $new_class_name ) . '{ font-size: '.$responsive_font_size.' !important; } } ';

if( $responsive_line_height != '' )
	$custom_inline_css .= '@media (max-width:767px){ .' . esc_attr( $new_class_name ) . '{ line-height: '.$responsive_line_height.' !important; } } ';

if( ! empty( $custom_inline_css ) ): ?>
<style type="text/css"><?php echo codeless_complex_esc( $custom_inline_css ) ?></style>
<?php endif; ?>
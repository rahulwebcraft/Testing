<?php
    ...
    if( is_archive() && ( ( function_exists( 'is_shop' ) && !is_shop() ) || !function_exists('is_shop') ) )
        $title = get_the_archive_title();

    if( function_exists( 'is_shop' ) && is_shop() && codeless_get_meta( 'page_header_title', '' ) == '' )
    ...
    if( is_home() && !is_page() )
        $title = esc_attr__('Home', 'regn');

    if( is_search() )
    $title = esc_attr__('Search Results', 'regn');
    
    $desc = codeless_get_meta( 'page_header_desc', '' );

    $style = codeless_get_layout_option( 'page_header_style', 'with_breadcrumbs' );
    ...
    if( is_page() || (is_single() && get_post_type() == 'portfolio' ) ){
    ...
    if( ( is_page() || (is_single() && get_post_type() == 'portfolio') ) && codeless_get_meta( 'page_header_default', 'default', get_the_ID() )  == 'custom' ){
    ...
    $text_color = codeless_get_layout_option( 'page_header_color', 'dark' );
    ...
    // Height
    $height = codeless_get_layout_option( 'page_header_height', '270' );
        ...
        <div class="cl-page-header__container <?php echo esc_attr( $page_header_container ) ?>">
            <div class="cl-page-header__row align-items-center row justify-content-between text-center">
                ...
                <div class="cl-page-header__breadcrumbs col-md-4 text-md-right">
                    <a href="<?php echo esc_url(home_url()) ?>" class="cl-page-header__breadcrumbs-link"><?php esc_html_e('Home', 'regn') ?></a>

		...
		<div class="cl-entry-single-comments">
			...
			<div class="cl-entry-single-comments__wrapper <?php echo esc_attr( $extra_container ) ?>">
				...
				<h6 class="cl-entry-single-section__title cl-custom-font">
					<?php
						...
						if( $comments_number == 1 )
							echo codeless_complex_esc( $comments_number ) . ' ' . esc_html__('Comment', 'regn');
						else
							echo codeless_complex_esc( $comments_number ) . ' ' . esc_html__('Comments', 'regn');
					?>
				</h6>
				...
				<?php the_comments_pagination( array(
					'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous', 'regn' ) . '</span>',
					'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next', 'regn' ) . '</span>',
				) );
			...
			if ( ! comments_open() && get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) : ?>
				...
				<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'regn' ); ?></p>
			<?php
			endif; ?>
		</div>
	</div>

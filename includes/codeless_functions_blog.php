...
function codeless_blog_pagination( $the_query = false, $id = '' ){
    ...
    echo '<div class="cl-pagination" data-container-id="'.esc_attr( $id ).'">';
...
function codeless_get_post_entry_meta(){
    ...
    if( codeless_get_mod( 'blog_entry_meta_categories', true ) ){
        ...
        $entry_meta_['prepend'] = esc_attr__( 'in', 'regn' );
    ...
    if( codeless_get_mod( 'blog_entry_meta_author', false ) ){
        ...
        $entry_meta_['prepend'] = esc_attr__( 'by', 'regn' );
...
function codeless_get_entry_meta_author(){
    ...
    if( empty( $author_name ) || is_customize_preview() )
        $author_name = esc_attr__( 'admin', 'regn' );
...
function codeless_get_entry_tool_comments_count(){
    ...
    $output = '<a href="' . get_permalink() . '#comments" class="comments"><span class="cl-entry__tool-count">' . ob_get_contents() . '</span><i class="cl-icon-comment-outline"></i></a>';
    ob_get_clean();
    return $output;
}
...
function codeless_get_entry_tool_likes(){
    ...
    if( class_exists('Codeless_Post_Like') )
        $output = Codeless_Post_Like::like('cl-icon-heart-outline');
...
function codeless_password_form( $post ){
    ...
    <p>' . esc_html__( 'This content is password protected. To view it please enter your password below:', 'regn' ) . '</p>
    <p><label for="' . $label . '">' . esc_html__( 'Password:', 'regn' ) . ' <input name="post_password" id="' . $label . '" type="password" size="20" /></label> <input type="submit" name="'.esc_attr__('Submit', 'regn').'" class="'.codeless_button_classes().'" value="' . esc_attr_x( 'Enter', 'post password form', 'regn' ) . '" /></p></form>
    ';

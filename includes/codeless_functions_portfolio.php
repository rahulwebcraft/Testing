...
function codeless_get_portfolio_thumbnail_size(){
    ...
    $portfolio = codeless_get_mod( 'portfolio_image_size', 'codeless_portfolio_entry' );
    
    if( codeless_get_mod( 'portfolio_module', 'isotope' ) == 'isotope' ){
        $meta = codeless_get_meta( 'portfolio_masonry_layout', 'small', get_the_ID() );
        $portfolio = 'codeless_portfolio_masonry_' . $meta;
    }

    if( codeless_get_mod( 'portfolio_module', 'isotope' ) == 'carousel' )
        $portfolio = 'codeless_portfolio_entry';

    if( codeless_get_mod( 'portfolio_image_size', 'codeless_portfolio_entry' ) != 'theme_default' )
        $portfolio = codeless_get_mod( 'portfolio_image_size', 'codeless_portfolio_entry' );
            ...
            <div class="portfolio_navigation"> 
                        ...
                        <?php if( is_object( get_previous_post() ) ): ?>
                            ...
                            <a class="cl-icon-arrow-left portfolio_single_left" href="<?php echo get_permalink(get_previous_post()->ID); ?>">
                                ...
                                <span><?php esc_attr_e('Prev', 'regn' ) ?></span>
                        ...
                        <?php if( is_object( get_next_post() ) ): ?>
                            ...
                            <a class="cl-icon-arrow-right portfolio_single_right" href="<?php echo get_permalink(get_next_post()->ID); ?>">
                                 ...
                                 <span><?php esc_attr_e('Next', 'regn' ) ?></span>
...
/**
 * Manage Classes of Portfolio Entries Div
 * @since 1.0.0
 */
function codeless_extra_classes_portfolio_entries( $classes ) {
    $classes[] = 'cl-portfolio';
    $classes[] = 'cl-portfolio--style-' . codeless_get_mod( 'portfolio_style', 'overlay1' );
    $classes[] = 'cl-portfolio--module-' . codeless_get_mod( 'portfolio_module', 'isotope' );


    if( codeless_get_mod( 'portfolio_module', 'isotope' ) == 'grid-blocks' )
        $classes[] = 'cl-portfolio--'. codeless_get_mod( 'portfolio_grid_block', 'grid-1' );

    if( codeless_get_mod( 'portfolio_animation', 'none' ) != 'none' )
        $classes[] = 'cl-portfolio--animated';

    if( codeless_get_mod( 'portfolio_image_gallery', 'no' ) == 'yes' )
        $classes[] = 'cl-portfolio--img-gallery';
    
    return $classes;
}

/**
 * Manage Attributes of Portfolio Entries
 * @since 1.0.0
 */
function codeless_extra_attr_portfolio_entries( $attr ) {
...
/**
 * Manage Classes of Portfolio Entries List
 * @since 1.0.0
 */
function codeless_extra_classes_portfolio_entries_list( $classes ) {

    if( codeless_get_mod( 'portfolio_module', 'none' ) == 'carousel' ){
        $classes[] = 'owl-carousel';
        $classes[] = 'cl-carousel';
        $classes[] = 'owl-theme';
        if( codeless_get_mod('portfolio_carousel_centered', 'no') == 'yes' )
            $classes[] = 'cl-carousel--centered';
    }
...
/**
 * Manage Attributes of Portfolio Entries List
 * @since 1.0.0
 */
function codeless_extra_attr_portfolio_entries_list( $attr ) {
    $attr[] = 'data-items="' . codeless_get_mod( 'portfolio_columns', '1' ) . '"';
    if( codeless_get_mod( 'portfolio_module', 'isotope' ) ){
        $attr[] = 'data-isotope-type="' . codeless_get_mod( 'portfolio_isotope_type', 'masonry' ) . '"';
    }

    if( codeless_get_mod( 'portfolio_carousel_nav', 'no' ) == 'yes' )
        $attr[] = 'data-nav="1"';
    ...
    if( codeless_get_mod( 'portfolio_module', 'isotope' ) == 'carousel' ){ 
        ...
        if( codeless_get_mod('portfolio_carousel_centered', 'no') == 'yes' ){
            $attr[] = 'data-responsive=\'{"0": {"stagePadding": 0}, "992": {"stagePadding": 400, "items": 1} }\'';
            $attr[] = 'data-center="1"';
            $attr[] = 'data-loop="1"';
        }else{
            $attr[] = 'data-responsive=\'{"0": {"items": 1}, "992": {"items": '.esc_attr( codeless_get_mod( 'portfolio_columns', '3' ) ).' } }\'';
        }
    }

    $attr[] = 'data-transition-duration="' . codeless_get_mod( 'portfolio_transition_duration', '0.4' ) . '"';
    return $attr;
}
...
function codeless_extra_classes_portfolio_item( $classes ) {
    ...
    if( codeless_get_mod( 'portfolio_module', 'isotope' ) == 'isotope' )
        $classes[] = 'cl-msn-size-' . codeless_get_meta( 'portfolio_masonry_layout', 'small', get_the_ID() );

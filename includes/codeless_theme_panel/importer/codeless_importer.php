...
class CodelessHeaderImporter{
	...
	public function process_install_header(){
		...
		wp_send_json_success( array('message' => esc_html__('Header Successfully imported', 'regn')) );
	}
	...
	// Read the file that will be written
		public function read_file_header($file, $dir){
			...
			if ( file_exists($file) ) {
				...
				$this->message = esc_html__("File doesn't exist", "regn");
			}
...
class CodelessImporter{
	...
	public function process_import_extra(){
		...
		wp_send_json_success( array('message' => esc_html__('Extra successfully imported', 'regn')) );
	}
	...
	public function process_import_widgets(){
		...
		if($options){
			...
			wp_send_json_success( array('message' => esc_html__('Widgets Successfully imported', 'regn')) );
			
		}else{
			wp_send_json_error( array('message' => esc_html__('Demo doesnt contain sidebar_widgets.txt file.', 'regn')) );
		}
	...
	public function process_import_options(){
		...
		if( $theme_mods && ! empty( $theme_mods ) ){
			...
			if( $options ){
				...
				wp_send_json_error( array('message' => esc_html__('Options not loaded or files missed.', 'regn'). $val) );
			}
		}
	...
	public function process_import_content(){
		...
		if(is_wp_error($value)){
			...
			wp_send_json_error( array('message' => esc_html__('Error. Content can\'t be installed.', 'regn') . $msg ) );
		}
		else {
			wp_send_json_success( array('message' => esc_html__('Content Successfully Installed', 'regn')) );
		}
	}
	...
	public function process_import_menus(){
		...
		if($menus_data){
			...
			wp_send_json_success( array('message' => esc_html__('Menu Successfully Installed', 'regn')) );

		}else{
			wp_send_json_error( array('message' => esc_html__('Error. Menu can\'t installed.', 'regn') ) );
		}
	}
	...
	public function ajax_plugins() {
			...
			$json['active'] = array(
						...
						'action2' => -1,
						'message' => esc_html__( 'Activating Plugin','regn' ),
			);
			...
			$json['install'] = array(
						...
						'action2' => -1,
						'message' => esc_html__( 'Installing Plugin','regn' ),
			);
			...
			if ( $json && ( !empty($json['active']['plugin']) || !empty($json['install']['plugin']) ) ) {
				...
				wp_send_json_success( array( 'message' => esc_html__( 'No plugins to install or activate', 'regn' ) ) );
			}
			exit;
...
function codeless_helper_import_footer( $data ){
	...
	if( $data && ! empty( $data ) ){
		...
		wp_send_json_error( array('message' => esc_html__('Options not loaded or files missed.', 'regn')) );
	}

	wp_send_json_success( array('message' => esc_html__('Footer Successfully imported', 'regn')) );	
}

...
class CodelessMostPopularWidget extends WP_Widget{
    ...
    function widget($atts, $instance){
        ...
        if( $type == 'popular' )
            ...
            $new_query = array( 'showposts' => $number_of_posts, 'orderby' => 'date', 'meta_query' => array(
                array(
                 'key' => '_thumbnail_id',
                 'compare' => 'EXISTS'
                ),
    ...
    function form($instance){
        ...
        <p>
            ...
            <select id="<?php echo esc_attr( $this->get_field_id('type') ); ?>" name="<?php echo esc_attr( $this->get_field_name('type')); ?>" value="<?php echo esc_attr($type); ?>">
                <option value="popular" <?php echo codeless_complex_esc( ($type == 'popular' ? 'selected="selected"': '' ) )?>><?php esc_attr_e('Popular', 'regn') ?></option>
                <option value="latest" <?php echo codeless_complex_esc(($type == 'latest' ? 'selected="selected"': '' ) )?>><?php esc_attr_e('Latest', 'regn') ?></option>
            </select>
            </label>

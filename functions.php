...
/**
 * Regn functions and definitions
 *
...
function codeless_setup(){
    ...
    add_action('wp_enqueue_scripts', 'codeless_register_global_styles', 99);
    ...
    if( function_exists( 'codeless_load_post_types' ) )
        codeless_load_post_types( array( 'codeless_slider', 'portfolio', 'staff', 'testimonial' ) );
...
function codeless_gutenberg_css(){
    ...
    $body_type = codeless_get_mod( 'body_typo', array( 'font-family' => 'Lato') );
    ...
    $dynamic_styles = '.editor-styles-wrapper{ -webkit-font-smoothing: antialiased !important; }';
    $dynamic_styles .= '.editor-styles-wrapper .wp-block-quote div p{ font-family: '.$body_type['font-family'].' !important; font-size:'.codeless_get_mod( 'blockquote_typo', array( 'font-size' => '24px' ) )['font-size'].' !important; font-weight:'.codeless_get_mod( 'blockquote_typo', array( 'font-weight' => '300' ) )['font-weight'].'; }';
    $dynamic_styles .= '.editor-styles-wrapper .wp-block-quote__citation{ font-weight: 500; font-style: normal; font-size:16px; }';
    $dynamic_styles .= '.editor-styles-wrapper{ font-family: '.$body_type['font-family'].' !important; line-height:'.$blog_text_size['line-height'].' !important;  }';
    ...
    $dynamic_styles .= '.editor-styles-wrapper p.has-drop-cap:not(:focus):first-letter { color: '.codeless_get_mod( 'primary_color', '#675478' ).'; } ';
    $dynamic_styles .= '.editor-styles-wrapper p:not(.has-text-color):not(.wp-block-cover-text), .editor-styles-wrapper .wp-block-quote__citation, .editor-styles-wrapper li { color:'.codeless_get_mod( 'text_color', '#8e999c' ).' !important; }'; 

    
    $dynamic_styles .= '.editor-styles-wrapper h1, .editor-styles-wrapper h2, .editor-styles-wrapper h3, .editor-styles-wrapper h4, .editor-styles-wrapper h5, .editor-styles-wrapper h6{ font-family:\'Montserrat\';}';
    $dynamic_styles .= '.editor-styles-wrapper h1{ font-size:'.codeless_get_mod( 'h1_typo', array( 'font-size' => '60px' ) )['font-size'].';font-weight:700 }';
    $dynamic_styles .= '.editor-styles-wrapper h2{ font-size:'.codeless_get_mod( 'h2_typo', array( 'font-size' => '42px' ) )['font-size'].'; font-weight:700}';
    $dynamic_styles .= '.editor-styles-wrapper h3{ font-size:'.codeless_get_mod( 'h3_typo', array( 'font-size' => '26px' ) )['font-size'].'; text-transform:uppercase;font-weight:700 }';
    $dynamic_styles .= '.editor-styles-wrapper h4{ font-size:'.codeless_get_mod( 'h4_typo', array( 'font-size' => '20px' ) )['font-size'].';font-weight:700 }';
    $dynamic_styles .= '.editor-styles-wrapper h5{ font-size:'.codeless_get_mod( 'h5_typo', array( 'font-size' => '16px' ) )['font-size'].'; font-weight:700}';
    $dynamic_styles .= '.editor-styles-wrapper h6{ font-size:'.codeless_get_mod( 'h6_typo', array( 'font-size' => '12px' ) )['font-size'].'; font-weight:700}';
...
function codeless_language_setup() {
    ...
    load_theme_textdomain('regn', $lang_dir);
} 
...
function codeless_theme_support(){
    ...
    add_theme_support(
		'editor-color-palette', array(
			array(
				'name'  => esc_html__( 'Primary Accent Color', 'regn' ),
				'slug' => 'primary-accent',
				'color' => codeless_get_mod( 'primary_color' ),
            ),
            array(
				'name'  => esc_html__( 'Labels Color', 'regn' ),
				'slug' => 'labels',
				'color' => codeless_get_mod( 'labels_color' ),
            ),
            array(
				'name'  => esc_html__( 'Heading Color', 'regn' ),
				'slug' => 'heading',
    ...
    add_theme_support( 'editor-font-sizes', array(
        array(
            'name' => esc_html__( 'Small', 'regn' ),
            'shortName' => esc_html__( 'S', 'regn' ),
            'size' => 14,
            'slug' => 'small'
        ),
        array(
            'name' => esc_html__( 'Normal', 'regn' ),
            'shortName' => esc_html__( 'M', 'regn' ),
            'size' => 18,
            'slug' => 'normal'
        ),
        array(
            'name' => esc_html__( 'Large', 'regn' ),
            'shortName' => esc_html__( 'L', 'regn' ),
            'size' => 20,
            'slug' => 'large'
        ),
        array(
            'name' => esc_html__( 'Huge', 'regn' ),
            'shortName' => esc_html__( 'XL', 'regn' ),
            'size' => 36,
            'slug' => 'larger'
        )
    ) );
...
function codeless_navigation_menus(){
    $navigations = array('main' => esc_html__('Main Navigation', 'regn') );
...
function codeless_register_global_styles(){ 
    ...
    wp_enqueue_style('codeless-theme-style', get_template_directory_uri() . '/css/theme.min.css');
    ...
    wp_enqueue_style('fancybox', CODELESS_BASE_URL.'css/jquery.fancybox.min.css' );
...
function codeless_register_global_scripts(){
   ...
   if( codeless_get_mod( 'codeless_page_transition', false )) 
    ...
    if( codeless_get_mod( 'nicescroll', 0 ) )
...
if( ! function_exists('codeless_header_logo_helper') ){
    function codeless_header_logo_helper(){
        ...
        if( codeless_get_mod('logo_type', 'font') == 'image' )
        {
            ...
            if(!empty($logo_url))
                $logo = "<img class='cl-logo__img--dark cl-logo__img' src=\"".esc_url($logo_url)."\" alt='".esc_attr__('Main Logo', 'regn')."' />";
            if( !empty( $logo_iphone ) )
                $logo .= "<img class='cl-logo__img cl-logo__img--iphone' src=\"".esc_url($logo_iphone)."\" alt='".esc_attr__('IPhone Logo', 'regn')."' />";
            if( !empty( $logo_ipad ) )
                $logo .= "<img class='cl-logo__img cl-logo__img--ipad' src=\"".esc_url($logo_ipad)."\" alt='".esc_attr__('IPad Logo', 'regn')."' />";

            if(!empty($logo_light_url))
                $logo_light = "<img class='cl-logo__img cl-logo__img--light' src=\"".esc_url($logo_light_url)."\" alt='".esc_attr__('Light Logo', 'regn')."' />";
        ...
        {   
            $logo = codeless_get_mod('logo_font_text', 'regn');
...
function codeless_get_team_thumbnail_size(){
    $team = codeless_get_mod( 'team_image_size', 'codeless_team_entry' );
    return $team;
}  
...
function codeless_image_sizes(){
    ...
    $default = array(
        'codeless_blog_entry'  => array(
			'label'   => esc_html__( 'Blog Entry', 'regn' ),
			'width'   => 'codeless_blog_entry_image_width',
			'height'  => 'codeless_blog_entry_image_height',
			'crop'    => 'codeless_blog_entry_image_crop',
			'section' => 'blog',
            'description' => esc_html__('Used as default for all blog images.', 'regn' ),
		),
        
        'codeless_blog_related'  => array(
			'label'   => esc_html__( 'Blog Related Posts', 'regn' ),
			'width'   => 'codeless_blog_related_image_width',
			'height'  => 'codeless_blog_related_image_height',
			'crop'    => 'codeless_blog_related_image_crop',
			'section' => 'blog',
            'description' => esc_html__('Used as default for related posts in single post.', 'regn' ),
            'defaults' => array('width'=> '360', 'height' => '300', 'crop' => 'center-center')
        ),
        
        'codeless_portfolio_entry'  => array(
			'label'   => esc_html__( 'Portfolio Entry', 'regn' ),
			'width'   => 'codeless_portfolio_entry_width',
			'height'  => 'codeless_portfolio_entry_height',
			'crop'    => 'codeless_portfolio_entry_crop',
			'section' => 'portfolio',
            'description' => esc_html__('Default Portfolio Entry Size', 'regn' ),
            //'defaults' => array('width'=> '90', 'height' => '57', 'crop' => 'center-center')
        ),
        
        'codeless_portfolio_masonry_small'  => array(
			'label'   => esc_html__( 'Portfolio Masonry Small Entry', 'regn' ),
			'width'   => 'codeless_portfolio_masonry_small_width',
			'height'  => 'codeless_portfolio_masonry_small_height',
			'crop'    => 'codeless_portfolio_masonry_small_crop',
			'section' => 'portfolio',
            'description' => esc_html__('Default Portfolio Masonry Small Entry Size', 'regn' ),
            'defaults' => array('width'=> '570', 'height' => '340', 'crop' => 'center-center')
        ),

        'codeless_portfolio_masonry_wide'  => array(
			'label'   => esc_html__( 'Portfolio Masonry Wide Entry', 'regn' ),
			'width'   => 'codeless_portfolio_masonry_wide_width',
			'height'  => 'codeless_portfolio_masonry_wide_height',
			'crop'    => 'codeless_portfolio_masonry_wide_crop',
			'section' => 'portfolio',
            'description' => esc_html__('Default Portfolio Masonry Wide Entry Size', 'regn' ),
            'defaults' => array('width'=> '1140', 'height' => '340', 'crop' => 'center-center')
        ),

        'codeless_portfolio_masonry_long'  => array(
			'label'   => esc_html__( 'Portfolio Masonry Long Entry', 'regn' ),
			'width'   => 'codeless_portfolio_masonry_long_width',
			'height'  => 'codeless_portfolio_masonry_long_height',
			'crop'    => 'codeless_portfolio_masonry_long_crop',
			'section' => 'portfolio',
            'description' => esc_html__('Default Portfolio Masonry Wide Entry Size', 'regn' ),
            'defaults' => array('width'=> '570', 'height' => '680', 'crop' => 'center-center')
        ),

        'codeless_team_entry'  => array(
			'label'   => esc_html__( 'Team Entry', 'regn' ),
			'width'   => 'codeless_team_entry_width',
			'height'  => 'codeless_team_entry_height',
			'crop'    => 'codeless_team_entry_crop',
			'section' => 'team',
            'description' => esc_html__('Default Team Entry Size', 'regn' ),
            'defaults' => array('width'=> '300', 'height' => '300', 'crop' => 'center-center')
        ),
    ...
    foreach( $custom as $new ){
        $default[$new] = array(
            'label'   => esc_html__( 'Custom', 'regn' ) . ': ' . $new,
...
function codeless_infinite_scroll( $type = '', $query = false ) {
	...
	$output = '';
	$output .= '<div class="cl-pagination-infinite '. $type .'" data-type="' . esc_attr( $type ) . '" data-end-text="' . esc_html__( 'No more posts to load', 'regn' ) . '" data-msg-text="' . esc_html__( 'Loading Content', 'regn' ) . '">';
		$output .= '<div class="older-posts">';
			$output .= get_next_posts_link( esc_html__( 'Older Posts', 'regn' ) .' &rarr;', $max_num_pages);
		$output .= '</div>';
		...
		if( $type == 'loadmore' )
		    $output .= '<button id="cl_load_more_btn" class="' . codeless_button_classes( array( 'cl-btn', 'cl-btn--color-alt', 'cl-btn--size-medium', 'cl-btn--style-square' ) ) . '">' . esc_html__( 'Load More', 'regn' ) . '</button>';
	$output .= '</div>';
...
function codeless_get_entry_tool_share(){
    ...
    if( !empty( $shares ) ){
        foreach( $shares as $social_id => $data ){
            $output .= '<a href="' . $data['link'] . '" title="'.esc_attr__('Social Share', 'regn').' ' . $social_id . '" target="_blank"><i class="' . $data['icon'] .'"></i></a>';
        }
    }
    $output .= '</div>';
...
function codeless_layout_bordered(){
    if( ! codeless_get_mod( 'bordered_layout', false ) )
        return;
    ?>
...
function codeless_update_comment_fields( $fields ) {
	...
	$label     = $req ? '*' : ' ' . esc_attr__( '(optional)', 'regn' );
	$aria_req  = $req ? "aria-required='true'" : '';

	$fields['author'] =
		'<p class="comment-form-author">
			<label for="author">' . esc_html__( "Name", "regn" ) . $label . '</label>
			<input id="author" name="author" type="text" placeholder="' . esc_attr__( "Jane Doe", "regn" ) . '" value="' . esc_attr( $commenter['comment_author'] ) .
		'" size="30" ' . $aria_req . ' />
		</p>';

	$fields['email'] =
		'<p class="comment-form-email">
			<label for="email">' . esc_html__( "Email", "regn" ) . $label . '</label>
			<input id="email" name="email" type="email" placeholder="' . esc_attr__( "name@email.com", "regn" ) . '" value="' . esc_attr( $commenter['comment_author_email'] ) .
		'" size="30" ' . $aria_req . ' />
		</p>';

	$fields['url'] =
		'<p class="comment-form-url">
			<label for="url">' . esc_html__( "Website", "regn" ) . '</label>
			<input id="url" name="url" type="url"  placeholder="' . esc_attr__( "http://google.com", "regn" ) . '" value="' . esc_attr( $commenter['comment_author_url'] ) .
		'" size="30" />
			</p>';
...
function codeless_update_comment_field( $comment_field ) {
    ...
    $comment_field =
      '<p class="comment-form-comment">
              <label for="comment">' . esc_html__( "Comment", "regn" ) . '</label>
              <textarea required id="comment" name="comment" placeholder="' . esc_attr__( "Enter comment here...", "regn" ) . '" cols="45" rows="8" aria-required="true"></textarea>
          </p>';
...
function codeless_get_animations_list($reverse = false){
    $animations = array(
        'none'	=> esc_html__('None', 'regn'),
        'top-t-bottom' =>	esc_html__('Top to Bottom', 'regn' ),
        'bottom-t-top' =>	esc_html__('Bottom to Top', 'regn' ),
        'right-t-left' => esc_html__('Right to Left', 'regn' ),
        'left-t-right' => esc_html__('Left to Right', 'regn' ),
        'alpha-anim' => esc_html__('Alpha Anim', 'regn' ),	
        'zoom-in' => esc_html__('Zoom In', 'regn' ),	
        'zoom-out' => esc_html__('Zoom Out', 'regn' ),	
        'zoom-reverse' => esc_html__('Zoom Reverse', 'regn' )
    );

    return $reverse ? array_flip($animations) : $animations;
}


function codeless_active_vc_portfolio(){
    $cap = get_role( 'administrator' )->capabilities;
    if( !isset( $cap['vc_access_rules_post_types/portfolio']) ){
        wp_roles()->add_cap( 'administrator', 'vc_access_rules_post_types', 'custom' );
        wp_roles()->add_cap( 'administrator', 'vc_access_rules_post_types/page', true );
        wp_roles()->add_cap( 'administrator', 'vc_access_rules_post_types/portfolio', true );
    }
} 

add_action('init', 'codeless_active_vc_portfolio' );
...
function codeless_dynamic_css_register_tags( $option = false, $suboption = false ){
    ...
    $data['primary_color']['color'] = array(
        ...
        '.cl-blog--style-default .cl-entry__meta--category a, .cl-blog--style-default .cl-entry__meta--author a',
        '.cl-highlight-text',
        '.cl-woo-product__overlay-buttons a.button, .woocommerce ul.products li.product .cl-woo-product__overlay-buttons .cl-view-details:hover, .cl-woo-product__overlay-buttons a.button:hover',
        '.price ins .woocommerce-Price-amount',
        '.woocommerce ul.products li.product .cl-woo-product__title-wrapper .price ins',
        '.cl-filters a.active, .cl-filters a:hover',
        ...
        '.cl-blog--style-noimage .cl-entry__meta a, .cl-blog--style-noimage .cl-entry__meta span, .cl-blog--style-noimage .cl-entry__meta',
        '.cl-table-row__text'
    );
    // Background Color
    ...
    $data['border_color'] = array(
        ...
        '.woocommerce table.shop_table .coupon #coupon_code.input-text',
...
add_filter( 'slider_templates_theme_included', 'slider_templates_included' );
function slider_templates_included(){
	return array(
        799,
        805,
        802,
